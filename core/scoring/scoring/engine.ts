...
import { type ComponentWeights, DEFAULT_WEIGHTS } from '@core/feedback/types';
...
function computeContextualExclusionPenalty(
  ...
  const SECTOR_KEYWORDS: Record<string, string[]> = {
    ...
    bureautique:  ['imprimante', 'cartouche', 'toner', 'papier', 'encre', 'ramette', 'enveloppe'],
 ...
 * @param weights  Multiplicateurs de composants (optionnel — défaut : tous à 1.0)
 *                 Provient du système de feedback learning pour personnaliser le scoring.
 *
...
export function scoreBC(
  ...
  // 3. Score total (avec multiplicateurs de feedback learning)
  const raw = (
    Math.round(titleR.score   * weights.title_multiplier)
    + Math.round(contentR.score * weights.content_multiplier)
    + Math.round(articleR.score * weights.article_multiplier)
    + Math.round(bizScore       * weights.business_intent_multiplier)
    + Math.round(techR.score    * weights.technical_multiplier)
    + Math.round(orgScore       * weights.organization_multiplier)
    + exclR.penalty  // la pénalité n'est pas pondérée
  );
